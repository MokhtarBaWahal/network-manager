...
class MikroTikDriver(BaseDeviceDriver):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(f"Connected to MikroTik router {self.device_id} at {self.ip_address}")
                self.connected = True
                return True
            else:
    ...
    async def get_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get firewall filter rules count
            ...
            firewall_rules = len(filter_response.json()) if filter_response.status_code == 200 else 0
            
            # Get NAT rules count
            nat_url = f"{self._get_base_url()}/ip/firewall/nat"
            nat_response = await self.client.get(nat_url, headers=self._get_auth_header())
            nat_rules = len(nat_response.json()) if nat_response.status_code == 200 else 0
            
            # Get DHCP server pools
            dhcp_url = f"{self._get_base_url()}/ip/pool"
            dhcp_response = await self.client.get(dhcp_url, headers=self._get_auth_header())
            dhcp_pools = dhcp_response.json() if dhcp_response.status_code == 200 else []
            ...
            logger.info(f"Retrieved config from MikroTik {self.device_id}")
            return config
        except Exception as e:
